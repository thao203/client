...
const ModalAddHandleBook = ({ visible, onCancel, onSuccess }) => {
  ...
  const cot = [
    ...
    },
    {
      ...
      key: 'action',
      render: (_, __, index) => (
        <Button type="link" danger onClick={() => xoaSach(index)} style={{ fontWeight: 500 }}>
          Xóa
        </Button>
      ),
    },
  ...
  const xuLyGuiYeuCau = async () => {
    ...
    try {
      ...
      const errorMessage =
        error.response?.data?.errors || error.response?.data?.message || error.message || 'Có lỗi xảy ra!';
  ...
  return (
    <StyledModal
      ...
      footer={[
        ...
        <SubmitButton key="submit" type="primary" onClick={xuLyGuiYeuCau} loading={dangTai}>
          Gửi yêu cầu
        </SubmitButton>,
      ]}
      width={960}
    ...
    >
      <StyledForm form={form} layout="vertical">
        ...
        <BookSelectionSection>
          <h4 style={{ marginBottom: '16px', fontSize: '16px', fontWeight: 600 }}>Thêm sách</h4>
          <div style={{ display: 'flex', gap: '12px', alignItems: 'flex-end' }}>
            ...
            >
              <InputNumber min={1} style={{ width: '100%', borderRadius: '6px', height: '40px' }} />
            </Form.Item>

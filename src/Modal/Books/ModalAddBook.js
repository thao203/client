  ...
  useEffect(() => {
    if (categorySearchQuery && categories.length > 0) {
      const filtered = categories.filter(
        (category) =>
          category && category.madanhmuc && category.madanhmuc.toLowerCase().includes(categorySearchQuery.toLowerCase())
      );
    ...
    <>
      <Modal show={showModalAddBook} onHide={handleClose} size="lg">
        <Modal.Header closeButton className="justify-content-center">
          <Modal.Title style={{ fontSize: '1.5rem', fontWeight: 'bold' }}>Thêm Sách</Modal.Title>
        </Modal.Header>

  ...
  useEffect(() => {
    if (locations.length > 0) {
      const filtered = locationSearchQueries.map((query) =>
        query
          ? locations.filter((location) => location?.mavitri?.toLowerCase().includes(query.toLowerCase()))
          : locations
      );
    ...
    <>
      <Modal show={showModalEditBook} onHide={handleClose} size="lg">
        <Modal.Header closeButton className="justify-content-center">
          <Modal.Title style={{ fontSize: '1.5rem', fontWeight: 'bold' }}>Sửa Sách</Modal.Title>
        </Modal.Header>
            ...
            <Row className="g-3">
              ...
              <Col md={6}>
                <Form.Group controlId="formMasach">
                  ...
                  <Form.Control type="text" value={masach} disabled className="shadow-sm" />
                </Form.Group>
              ...
              <Col md={12}>
                ...
                {vitri.map((loc, index) => (
                  <Row key={index} className="mb-2 align-items-center" style={{ position: 'relative' }}>
                    ...
                    <Col md={2}>
                      <Button variant="outline-danger" onClick={() => handleRemoveLocation(index)} className="w-100">
                        Xóa
                      </Button>
                    </Col>
                  </Row>

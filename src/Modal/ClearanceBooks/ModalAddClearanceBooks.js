...
function ModalAddClearanceBooks({ showModalAddClearanceBooks, setShowModalAddClearanceBooks, onAddSuccess }) {
  ...
  const isFormValid = () => masachthanhly && masach && soluong && mavitri && lydo && Number(soluong) > 0;
  ...
  const handleAddClearanceBook = async () => {
    ...
    try {
      ...
      const errorMessage = error.response?.data?.message || 'Có lỗi xảy ra khi thêm sách thanh lý!';
    ...
    <>
      <Modal show={showModalAddClearanceBooks} onHide={handleClose} size="lg">
        <Modal.Header closeButton className="justify-content-center">
          <Modal.Title style={{ fontSize: '1.5rem', fontWeight: 'bold' }}>Thêm Sách Thanh Lý</Modal.Title>
        </Modal.Header>
        ...
        <Modal.Footer className="justify-content-between">
          ...
          <Button variant="primary" onClick={handleAddClearanceBook} className="px-4" disabled={!isFormValid()}>
            Thêm Mới
          </Button>
        </Modal.Footer>
      </Modal>

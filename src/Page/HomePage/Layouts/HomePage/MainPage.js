...
import { NewReleases, Info, ContactMail, Help, Star, FormatQuote } from '@mui/icons-material';
...
const CategorySection = ({ onCategoryClick }) => {
  const categories = [
    {
      icon: <NewReleases sx={{ fontSize: 40, color: 'white' }} />,
      title: 'Sách mới',
      bgColor: '#E91E63',
      section: 'featured',
    },
    ...
    {
      icon: <FormatQuote sx={{ fontSize: 40, color: 'white' }} />,
      title: 'Danh ngôn',
      bgColor: '#FF5722',
      section: 'quote',
    },
...
const BookCardItem = ({ book, onViewDetail }) => (
  <BookCard initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.3 }}>
    <Box sx={{ height: 250, bgcolor: '#f5f5f5', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <img src={book.img} alt={book.tensach} style={{ maxHeight: '100%', maxWidth: '100%', objectFit: 'contain' }} />
    </Box>
    <Box p={2}>
...
const QuoteBookSection = ({ sectionRef }) => {
  const quotes = [
    {
      quote: 'Cuộc sống giống như một cuốn sách. Có những chương buồn, nhưng cũng có những chương đầy niềm vui.',
      author: 'Khuyết danh',
    },
  ...
  const guides = [
    {
      ...
      title: 'Giới Thiệu',
      desc: 'Thông tin chi tiết về hệ thống',
      link: '/about',
    },
    {
      icon: <ContactMail sx={{ fontSize: 40, color: 'primary.main' }} />,
      title: 'Liên Hệ',
      desc: 'Thông tin liên hệ hỗ trợ',
      link: '/contact',
    },
    {
      icon: <Help sx={{ fontSize: 40, color: 'primary.main' }} />,
      title: 'Hướng Dẫn',
      desc: 'Hướng dẫn sử dụng chi tiết',
      link: '/guide',
    },
  ];
      ...
      >
        {guides.map((guide, index) => (
          ...
          >
            ...
            <Button variant="outlined" color="primary" size="small" component={Link} to={guide.link} sx={{ mt: 2 }}>
              Xem Thêm
            </Button>
          </GuideCard>
        ))}

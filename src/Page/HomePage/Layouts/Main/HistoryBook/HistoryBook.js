...
function HistoryBook() {
  ...
  const handleExtendAllBooks = async (maphieumuon) => {
    try {
      ...
      const booksToExtend = borrowingSlip.books.filter((book) => !book.tinhtrang && !book.giahan);
    ...
    <div className={cx('wrapper')}>
                    ...
                    return (
                      <Fade in timeout={400 + index * 100} key={index}>
                        <StyledTableRow>
                          ...
                          <StyledTableCell>
                            {hasOverdue ? (
                              <Chip label={`${maxOverdueDays} ngày`} color="error" size="small" />
                            ) : (
                              'Không'
                            )}
                          </StyledTableCell>
                            ...
                            <div style={{ display: 'flex', gap: '8px' }}>
                              ...
                              !item.books.every((book) => book.giahan) ? (
                                <MuiButton
                                  variant="contained"
                                  size="small"
                                  onClick={() => handleExtendAllBooks(item?.maphieumuon)}
                                  sx={{
                                    borderRadius: 20,
                                    textTransform: 'none',
                              ...
                              <MuiButton
                                variant="outlined"
                                size="small"
                                onClick={() => handleOpenModal(item.books, item.maphieumuon)}
                                sx={{
                                  borderRadius: 20,
                                  textTransform: 'none',
                                  fontSize: { xs: '0.7rem', sm: '0.875rem' },
      ...
      <Modal open={openModal} onClose={handleCloseModal}>
        <ModalBox>
          ...
          <Table>
            ...
            <TableBody>
              {selectedBooks.map((book, index) => {
                const dueDate =
                  book.ngayhentra || dataBooks.find((item) => item.maphieumuon === selectedMaphieumuon)?.ngayhentra;
                ...
                return (
                  <TableRow key={index}>
                    ...
                    <StyledTableCell>
                      {overdueDays > 0 ? <Chip label={`${overdueDays} ngày`} color="error" size="small" /> : 'Không'}
                    </StyledTableCell>
                    <StyledTableCell>{book.ngaytra ? formatDate(book.ngaytra) : 'Chưa trả'}</StyledTableCell>
                    <StyledTableCell>
                      {book.giahan && book.newngayhentra ? formatDate(book.newngayhentra) : 'N/A'}
                    </StyledTableCell>
                    <StyledTableCell>
                      {book.tinhtrang ? (
                        <Chip icon={<FontAwesomeIcon icon={faCheck} />} label="Đã trả" color="success" size="small" />
                      ) : (
                        <Chip
                          icon={<FontAwesomeIcon icon={faSpinner} className="fa-spin" />}
                          label="Đang mượn"
          ...
          <MuiButton variant="contained" onClick={handleCloseModal} sx={{ mt: 2, float: 'right' }}>
            Đóng
          </MuiButton>
        </ModalBox>
      </Modal>

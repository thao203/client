...
import { Box, Typography, Button, IconButton, Paper, Divider, Checkbox, FormControlLabel } from '@mui/material';
...
const Cart = () => {
  ...
  const handleAddToCart = (cartItem) => {
    const existingItem = cartItems.find((item) => item.masach === cartItem.masach && item.mavitri === cartItem.mavitri);
    if (existingItem) {
      setCartItems(
        cartItems.map((item) =>
  ...
  const increaseQuantity = (masach, mavitri) => {
    setCartItems(
      cartItems.map((item) =>
        item.masach === masach && item.mavitri === mavitri ? { ...item, soluong: item.soluong + 1 } : item
      )
    );
  };
  ...
  const removeItem = (masach, mavitri) => {
    setCartItems(cartItems.filter((item) => !(item.masach === masach && item.mavitri === mavitri)));
  ...
  const handleCheckout = async () => {
    try {
      const selectedCartItems = cartItems.filter((item) => selectedItems.includes(`${item.masach}-${item.mavitri}`));
      ...
      if (allSuccessful) {
        ...
        setTimeout(() => {
          setCartItems(cartItems.filter((item) => !selectedItems.includes(`${item.masach}-${item.mavitri}`)));
          setSelectedItems([]);
              ...
              <Box>
                ...
                {cartItems.map((item) => (
                  <CartPaper key={`${item.masach}-${item.mavitri}`} elevation={0}>
                    <CartItem isSelected={selectedItems.includes(`${item.masach}-${item.mavitri}`)}>
                      <Checkbox
                        checked={selectedItems.includes(`${item.masach}-${item.mavitri}`)}
                        onChange={() => toggleItemSelection(item.masach, item.mavitri)}
                        color="primary"
                      />
                      <Box
                        component="img"
                      ...
                      <QuantityControls>
                        <IconBtn
                          size="small"
                          onClick={() => decreaseQuantity(item.masach, item.mavitri)}
                          disabled={item.soluong <= 1}
                        >
                          <RemoveIcon />
                        </IconBtn>
                        <Typography sx={{ minWidth: 30, textAlign: 'center' }}>{item.soluong}</Typography>
                        <IconBtn size="small" onClick={() => increaseQuantity(item.masach, item.mavitri)}>
                          <AddIcon />
                        </IconBtn>
                        <IconBtn
                          size="small"
                          onClick={() => removeItem(item.masach, item.mavitri)}
                          sx={{
                ...
                <Box sx={{ textAlign: 'right' }}>
                  <StyledButton variant="contained" onClick={handleCheckout}>
                    Mượn sách
                  </StyledButton>
                </Box>
              </Box>

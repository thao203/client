      ...
      <Slide direction="up" in timeout={800}>
        ...
        >
          {currentBooks.map((item, index) => {
            ...
            return (
              <Grow key={item.masach} in timeout={500 + index * 150} style={{ transformOrigin: 'bottom center' }}>
                <StyledCard onMouseEnter={() => setHoveredCard(item.masach)} onMouseLeave={() => setHoveredCard(null)}>
                  <StyledCardMedia
                    component="img"
                    image={item.img}
                    alt={item.tensach}
                  ...
                  {isHovered && (
                    <Fade in={isHovered} timeout={300}>
                      <CartButton onClick={() => handleShowCart(item.masach, item.tensach, item.vitri)}>

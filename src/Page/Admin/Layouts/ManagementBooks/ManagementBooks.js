...
function ManagementBooks() {
  ...
  const categories = [...new Set(dataBooks.map((book) => book.tendanhmuc).filter(Boolean))].map((category) => ({
    value: category,
    label: category,
  }));
  const publishers = [...new Set(dataBooks.map((book) => book.nhaxuatban).filter(Boolean))].map((publisher) => ({
    value: publisher,
    label: publisher,
  }));
  ...
  return (
    <div className={cx('wrapper')}>
                  ...
                  <tr key={item.masach}>
                    ...
                    <td>
                      <div className="d-flex gap-2 justify-content-center">
                        <button onClick={() => handleShowDetail(item.masach)} className="btn btn-info btn-sm">
                          <i className="bi bi-eye me-1" /> Xem
                        </button>
                        <button onClick={() => handleShow1(item.masach)} className="btn btn-warning btn-sm">
                          <i className="bi bi-pencil me-1" /> Sửa
                        </button>
                        <button onClick={() => handleShow2(item.masach)} className="btn btn-danger btn-sm">
      ...
      <ModalDeleteBook
        ...
        masach={masachBook}
      />
      <DetailBookModal open={showDetailModal} onClose={handleCloseDetail} masach={selectedMasach} />
    </div>
  );
}

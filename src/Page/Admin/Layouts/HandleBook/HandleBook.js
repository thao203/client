...
function HandleBook() {
  ...
  const updateBorrowedRecords = (maphieumuon, updatedBooks) => {
    setBorrowedRecords((prevRecords) =>
      prevRecords.map((record) => {
        if (record.maphieumuon === maphieumuon) {
          const updatedRecord = {
            ...record,
            books: record.books.map((book) => {
              const updatedBook = updatedBooks.find((b) => b.masach === book.masach);
              if (updatedBook) {
                return {
                  ...book,
                  isExtended: updatedBook.giahan || false,
                  newngayhentra: updatedBook.newngayhentra || book.newngayhentra,
                  ngayhentra: updatedBook.ngayhentra || book.ngayhentra,
                  overdueDays: calculateOverdueDays(
                    updatedBook.newngayhentra || updatedBook.ngayhentra,
                    book.tinhtrang,
                    book.overdueDays
                  ),
                };
          ...
          // Đặt isExtended thành true chỉ khi tất cả sách chưa trả đã gia hạn
          updatedRecord.isExtended = updatedRecord.books.every((book) => book.tinhtrang || book.isExtended);
  ...
  const handleFilterSubmit = (e) => {
    ...
    if (filterDueStartDate && filterDueEndDate && new Date(filterDueStartDate) > new Date(filterDueEndDate)) {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        if (updatedBooks.length === 0) {
          ...
          updateBorrowedRecords(maphieumuon, updatedBooks);
  ...
  const handleExtendBorrowing = async (maphieumuon, masach) => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        updateBorrowedRecords(maphieumuon, [updatedBook]);
      ...
      <div className="container my-4">
        <div className="row align-items-center justify-content-between g-3">
          ...
          <div className="col-12 col-md-3 text-center text-md-end">
            ...
            <button onClick={() => setShowFilterForm(!showFilterForm)} className="btn btn-secondary">
          ...
          <div className="card p-4">
            ...
            <form onSubmit={handleFilterSubmit}>
              ...
              <div className="mt-4 d-flex gap-2">
                ...
                <button type="button" className="btn btn-outline-secondary" onClick={handleFilterReset}>
                  Xóa bộ lọc
                </button>
              </div>
            </form>
              ...
              {currentItems.length > 0 ? (
                currentItems.map((record) => (
                  <tr key={record.maphieumuon}>
                    ...
                    <td className={getOverdueStatus(record.books, record) === 'Có' ? 'text-danger fw-bold' : ''}>
                      ...
                      {record.books.some((book) => book.confirm && !book.tinhtrang) ? (
                        <div className="d-flex gap-2 flex-wrap">
                          ...
                          {record.books.every((book) => book.tinhtrang || book.isExtended) ? (
                            <span className="text-info">Đã Gia Hạn Tất Cả</span>
                          ) : (
                            <button
                        ...
                        <span>
                          {formatDate(record.books.find((book) => book.tinhtrang)?.ngaytra || record.ngaytra)}
                          <button
            ...
            <ul className="pagination justify-content-center mt-3">
              ...
              {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
                <li key={page} className={`page-item ${currentPage === page ? 'active' : ''}`}>
                  <button className="page-link" onClick={() => handlePageChange(page)} disabled={isLoading}>
                    {page}
                  </button>
                </li>
              ))}
          ...
          {selectedRecord && (
            <div>
              ...
              <p>
                ...
                <span className={calculateFine(selectedRecord.books, selectedRecord) > 0 ? 'text-danger fw-bold' : ''}>
                    ...
                    {selectedRecord.books.map((book) => {
                      ...
                      return (
                        <tr key={book.masach}>
                          ...
                          <td className={overdueDays > 0 ? 'text-danger fw-bold' : ''}>
                          ...
                          <td>
                            {book.confirm && !book.tinhtrang ? (
                              <div className="d-flex gap-2">
                                <button
                                  className="btn btn-warning btn-sm"
                                  onClick={() => handleReturnBook(selectedRecord.maphieumuon, book.masach)}
                                  disabled={isLoading}
                                >
                                  Trả Sách
                                </button>
                                {book.isExtended ? (
                                  ...
                                  <button
                                    className="btn btn-info btn-sm"
                                    onClick={() => handleExtendBorrowing(selectedRecord.maphieumuon, book.masach)}
                                    disabled={isLoading}
                                  >
                                    Gia Hạn
                                  </button>
